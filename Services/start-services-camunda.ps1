# Script para iniciar servicios con Camunda en la terminal actual de VS Code
Write-Host "===============================================================" -ForegroundColor Cyan
Write-Host "INICIANDO SERVICIOS SOA CON CAMUNDA" -ForegroundColor Cyan
Write-Host "===============================================================" -ForegroundColor Cyan

$basePath = "d:\Tareas de programacion\SOA"

Write-Host "`nIniciando servicios como Jobs en PowerShell..." -ForegroundColor Yellow
Write-Host "Los logs estaran disponibles en archivos .log`n" -ForegroundColor Gray
...
Write-Host "[3/7] Iniciando camunda-service (puerto 8083)..." -ForegroundColor Magenta
Start-Job -Name "camunda-service" -ScriptBlock {
    Set-Location "d:\Tareas de programacion\SOA\camunda-service"
    java -jar target\camunda-service-0.0.1-SNAPSHOT.jar 2>&1 | Tee-Object -FilePath "..\logs\camunda-service.log"
} | Out-Null
Start-Sleep -Seconds 12
...
Write-Host "[6/7] Iniciando image-service (puerto 8087)..." -ForegroundColor Cyan
Start-Job -Name "image-service" -ScriptBlock {
    Set-Location "d:\Tareas de programacion\SOA\image-service"
    java -jar target\image-service-0.0.1-SNAPSHOT.jar 2>&1 | Tee-Object -FilePath "..\logs\image-service.log"
} | Out-Null
Start-Sleep -Seconds 12

Write-Host "[7/8] Iniciando ticket-service (puerto 8086)..." -ForegroundColor Cyan
...
Write-Host "[8/8] Iniciando gateway (puerto 8080)..." -ForegroundColor Cyan
...
Write-Host "`nServicios activos: $($ports.Count)/8" -ForegroundColor $(if ($ports.Count -eq 8) { "Green" } else { "Yellow" })
if ($ports.Count -gt 0) {
    Write-Host "Puertos escuchando: $($ports -join ', ')" -ForegroundColor Green
}

if ($ports.Count -eq 8) {
    Write-Host "`nTODOS LOS SERVICIOS INICIADOS CORRECTAMENTE`n" -ForegroundColor Green
    Write-Host "Camunda Cockpit: http://localhost:8083/camunda" -ForegroundColor Magenta
    Write-Host "Usuario: admin | Password: admin`n" -ForegroundColor Gray
} else {
    Write-Host "`nAlgunos servicios estan iniciando, espera unos segundos mas" -ForegroundColor Yellow
...
Write-Host "  Get-Content logs\camunda-service.log -Tail 20 -Wait" -ForegroundColor White
Write-Host ""
...
Write-Host ""
Write-Host "Camunda Cockpit:" -ForegroundColor Yellow
Write-Host '  http://localhost:8083/camunda (admin/admin)' -ForegroundColor White
Write-Host ""
